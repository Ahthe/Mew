        ...
        return (
          ...
          >
            {item.type === "session" ? (
              ...
              <span
                className="mention-type-icon"
                style={{
                  width: 12,
                  height: 12,
                  borderRadius: "50%",
                  background: item.content ?? "#888",
                  flexShrink: 0,
                }}
              />
            ) : null}
...
const suggestion = (
  ...
  return {
    ...
    items: async ({ query, editor }) => {
      ...
      // When cursor is inside a heading, show goals for binding
      if (config.goals && isNodeActive(editor.state, "heading")) {
        const q = normalizedQuery.toLowerCase();
        const filtered = q
          ? config.goals.filter((g) => g.label.toLowerCase().includes(q))
          : config.goals;
...
export interface MentionConfig {
  trigger: string;
  handleSearch: (query: string) => Promise<MentionItem[]> | MentionItem[];
  goals?: MentionItem[];
}
...
function MentionAvatar({
  ...
  if (type === "human") {
    ...
    return (
      <span className="mention-avatar mention-avatar-human">
        {initial}
      </span>
    );
  }
...
function MentionNodeView({ node }: NodeViewProps) {
  ...
  return (
    <NodeViewWrapper
      as="a"
      className={cn(["mention", mentionType === "human" && "mention-human"])}
      data-mention="true"
      data-id={mentionId}
  ...
  return Mention.extend({
    ...
    renderHTML: ({ node }) => {
      ...
      return [
        "a",
        {
          class: `mention${type === "human" ? " mention-human" : ""}`,
          "data-mention": `true`,
          "data-id": id,
          "data-type": type,
          "data-label": label,
        ...
        ["span", { class: "mention-avatar mention-avatar-human" }, initial],
