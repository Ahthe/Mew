  ...
  return (
    <div className="bg-[#f4efe9]">
      <div className="flex w-full min-w-0 flex-col items-center text-center">
        <section
          id="hero"
          className="laptop:px-0 flex w-full min-w-0 flex-col items-center gap-10 px-4 pt-20 pb-10 text-center"
        >
          <div className="flex w-full max-w-3xl min-w-0 flex-col gap-5">
            <h1 className="min-w-0 font-serif text-3xl leading-tight break-words text-[#2a2622] sm:text-6xl">
              {heroContent.title}
            </h1>
            <p className="mx-auto max-w-2xl min-w-0 text-base break-words text-[#817b74] sm:text-xl">
...
function CharHomeMockup() {
  ...
  const dateNumber = now.toLocaleDateString("en-US", { day: "numeric" });
  const month = now.toLocaleDateString("en-US", { month: "long" });

  return (
    <div className="w-full px-4 pb-12">
      <div className="mx-auto flex h-[520px] max-w-5xl overflow-hidden rounded-[18px] border border-[#eadfd7] bg-white text-left shadow-[0_30px_80px_-45px_rgba(64,44,32,0.45)]">
        <aside className="hidden w-72 shrink-0 flex-col border-r border-[#eadfd7] bg-[#f7f2ee] p-3 md:flex">
          <div className="mb-5 flex h-7 items-center gap-2">
          ...
          <div className="mb-3 text-[11px] font-semibold tracking-[0.14em] text-[#e45745] uppercase">
            Today
          </div>
          {[
            ["Product sync", "9:30 AM"],
            ["Customer notes", "11:10 AM"],
            ["Hiring loop", "2:00 PM"],
            ["Design review", "4:15 PM"],
          ].map(([title, time], index) => (
            <div
              key={title}
              className={cn([
                "rounded-lg px-3 py-2",
                index === 0 ? "bg-[#eee3da]" : "hover:bg-white/60",
              ])}
            >
              <div className="truncate text-sm font-medium text-[#2a2622]">
                {title}
              </div>
              <div className="text-xs text-[#9b948b]">{time}</div>
            </div>
          ))}
        </aside>
        <div className="min-w-0 flex-1 bg-white">
          <div className="flex h-10 items-center gap-2 border-b border-[#f0e7e0] px-4 md:hidden">
            <span className="size-3 rounded-full border border-black/10 bg-[#ff5f57]" />
            <span className="size-3 rounded-full border border-black/10 bg-[#ffbd2e]" />
            <span className="size-3 rounded-full border border-black/10 bg-[#28c840]" />
          </div>
          <div className="mx-auto flex h-full max-w-2xl flex-col px-6 py-8 sm:px-10">
            <div className="border-b border-[#f0e7e0] pb-7">
              <div className="text-[13px] font-medium tracking-wide text-[#e45745]">
                {day}
              </div>
              <div className="mt-2 font-serif text-5xl leading-tight text-[#2a2622]">
                Good morning.
              </div>
              <p className="mt-2 text-[15px] text-[#817b74]">
                A calm place for meetings, notes, transcripts, and follow-ups.
              </p>
            </div>

            <div className="flex items-baseline justify-between gap-4 py-6">
              <div>
                <div className="font-serif text-2xl text-[#2a2622]">Today</div>
                <div className="text-sm text-[#9b948b]">{month}</div>
              </div>
              <div className="font-serif text-5xl leading-none text-[#e45745]">
                {dateNumber}
              </div>
            </div>

            <MockSection label="Today's meetings">
              <MockRow title="Product sync" meta="9:30 AM" />
              <MockRow title="Design review" meta="4:15 PM" />
            </MockSection>
            <MockSection label="Notes">
              <MockRow title="Customer feedback themes" meta="11:10 AM" />
              <MockRow title="Launch blockers" meta="Yesterday" />
            </MockSection>
            <MockSection label="Tasks">
              <MockTask title="Send recap to the team" />
              <MockTask title="Turn transcript into action items" done />
            </MockSection>
          </div>
        </div>
      </div>
...
function MockSection({
  ...
  return (
    <div className="mb-5">
      <div className="mb-1 text-[11px] font-semibold tracking-[0.14em] text-[#b7afa6] uppercase">
        {label}
      </div>
      <div className="divide-y divide-[#f5eee9]">{children}</div>
    </div>
  );
}
...
function MockRow({ title, meta }: { title: string; meta: string }) {
  return (
    <div className="flex items-center justify-between gap-4 py-2 text-sm">
      <span className="truncate text-[#2a2622]">{title}</span>
      <span className="shrink-0 text-xs text-[#9b948b]">{meta}</span>
    </div>
  );
}

function MockTask({ title, done = false }: { title: string; done?: boolean }) {
  return (
    <div className="flex items-center gap-2.5 py-2 text-sm">
      <span
        className={cn([
          "size-4 rounded-[5px] border",
          done ? "border-[#e45745] bg-[#e45745]" : "border-[#c9c1b8]",
        ])}
      />
      <span className={done ? "text-[#9b948b] line-through" : "text-[#2a2622]"}>
        {title}
      </span>
    </div>
  );
