...
import { HeatmapNodeViewComponent } from "~/shared/goal/HeatmapNodeView";
...
>(({ sessionId, onNavigateToTitle }, ref) => {
  ...
  const mentionConfig = useMemo(
    () => ({
      trigger: "@",
      goals: Object.entries(goals).map(([rowId, g]) => ({
        id: rowId,
        type: "goal" as const,
    ...
    [search, sessions, humans, organizations, goals],
  );
  ...
  const extensionOptions = useMemo(
    () => ({
      ...
      heatmapExtension: HeatmapNode.extend({
        addNodeView() {
          return ReactNodeViewRenderer(HeatmapNodeViewComponent);
        },
      }),
    }),
    [],
