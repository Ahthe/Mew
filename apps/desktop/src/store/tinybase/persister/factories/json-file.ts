...
import { getStorageAdapter, type JsonValue } from "~/store/tinybase/storage";
...
>(
  ...
  try {
    ...
    const baseResult = await adapter.vaultBase();
    ...
    const path = [base, filename].join(adapter.pathSep());
    const result = await adapter.writeJsonBatch([[data as JsonValue, path]]);
...
function addListener(
  ...
  if (listenMode === "notify" || listenMode === "both") {
    (async () => {
      const unlisten = await getStorageAdapter().onFileChanged((path) => {
...
async function loadTableData(
  ...
  const base = baseResult.data;
  const path = [base, filename].join(adapter.pathSep());
  const result = await adapter.readTextFile(path);
