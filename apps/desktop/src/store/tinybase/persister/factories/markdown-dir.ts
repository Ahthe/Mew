...
async function loadMarkdownDir<TStorage extends Record<string, unknown>>(
  ...
  const result = await getStorageAdapter().readDocumentBatch(dir);
...
>(
  ...
  const readResult = await getStorageAdapter().readTextFile(filePath);
  ...
  const parseResult = await getStorageAdapter().deserialize(readResult.data);
