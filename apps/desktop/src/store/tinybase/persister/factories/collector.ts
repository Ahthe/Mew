...
async function writeJsonBatch(
  ...
  const result = await getStorageAdapter().writeJsonBatch(items);
...
async function writeDocumentBatch(
  ...
  const result = await getStorageAdapter().writeDocumentBatch(items);
  if (result.status === "error") {
...
async function deleteFiles(paths: string[], label: string): Promise<void> {
  ...
  for (const path of paths) {
    const result = await getStorageAdapter().remove(path);
    if (result.status === "error") {
...
async function countItemsOnDisk(config: OrphanCleanupConfig): Promise<number> {
  try {
    ...
    if (config.type === "dirs") {
      const result = await getStorageAdapter().scanAndRead(
        subdir,
        [config.markerFile],
        true,
        null,
...
async function runOrphanCleanup(
  ...
  for (const config of configs) {
    ...
    try {
      if (config.type === "dirs") {
        await getStorageAdapter().cleanupOrphan(
          {
            type: "dirs",
