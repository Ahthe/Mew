...
import { getStorageAdapter } from "~/store/tinybase/storage";
...
export async function loadAllChatGroups(
  ...
  const scanResult = await getStorageAdapter().scanAndRead(
    chatsDir,
    [CHAT_MESSAGES_FILE],
    false,
    null,
...
export async function loadSingleChatGroup(
  ...
  const result = await getStorageAdapter().readTextFile(filePath);
