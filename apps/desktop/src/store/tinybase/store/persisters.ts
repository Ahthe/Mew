...
import { useGoalContributionPersister } from "~/store/tinybase/persister/goal-contribution";
import { useGoalPersister } from "~/store/tinybase/persister/goal";
...
export function useMainPersisters(store: Store) {
  ...
  const goalPersister = useGoalPersister(store);
  const goalContributionPersister = useGoalContributionPersister(store);
  ...
  useEffect(() => {
    ...
    const persisters = [
      ...
      { id: "goal", persister: goalPersister },
      { id: "goalContribution", persister: goalContributionPersister },
    ];
